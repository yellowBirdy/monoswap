...
transaction(name: String, source: String) {
    ...
    {
        acc.contracts.update__experimental(name: name,  code: source.decodeHex())
...
func main() {
	...
	proposerAddress := flow.HexToAddress(os.Getenv("MONOSWAP_ADD"))
	proposerKeyIndex := 0
  ...
  //
	payerAddress := flow.HexToAddress(os.Getenv("MONOSWAP_ADD"))
	authorizerAddress1 := flow.HexToAddress(os.Getenv("MONOSWAP_ADD"))
	////--
//
//
//
	...
	name := cadence.NewString("MonoswapFTPair")
	txt, err := ioutil.ReadFile("./contracts/exchange.cdc")
	if err != nil {
		panic("Could not read contract file")
	}
	//source := cadence.NewString("636f6e74726163742042616261207b7075622066756e206772656574202829207b6c6f6728224772656574696e67732068756d616e2122297d20696e697428297b7d207d0a")
	...
	var privKeyString = os.Getenv("MONOSWAP_KEY")
