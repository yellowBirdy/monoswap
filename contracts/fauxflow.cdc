...
pub contract FauxFlow: FungibleToken {
    ...
    init() {
        ...
        //temporarily setting admin account to contract account
        let adminAccount = self.account
        // Create the Vault with the total supply of tokens and save it in storage
        //
        let vault <- create Vault(balance: self.totalSupply)
        adminAccount.save(<-vault, to: /storage/FauxFlowVault)

        // Create a public capability to the stored Vault that only exposes
        // the `deposit` method through the `Receiver` interface
        //
        adminAccount.link<&FauxFlow.Vault{FungibleToken.Receiver}>(
        ...
        //
        adminAccount.link<&FauxFlow.Vault{FungibleToken.Balance}>(
            /public/FauxFlowBalance,
            target: /storage/FauxFlowVault
        )

        let admin <- create Administrator()
        adminAccount.save(<-admin, to: /storage/FauxFlowAdmin)
